...
export function initializeProviders(): void {
  ...
  // Register the OpenAI-compatible provider (default provider).
  providerRegistry.register(createOpenAIProvider());

  // Optional Gemini provider — registered only when GEMINI_API_KEY is set (opt-in, cloud).
  // The provider-agnostic seam made concrete: a second real provider behind the same interface.
